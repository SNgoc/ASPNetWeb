...
namespace HomeWork2_Item.Controllers
{
    public class ItemController : Controller
    {
        ...
        {
            return View("Login");//trả về trang login khi truy cập /Item
        }
        ...
        public async Task<IActionResult> ListItem(ItemModel item)
        {
            if (ModelState.IsValid)//check lỗi validation
            {
                ...
                var newItem = new Item { ItemCode = item.ItemCode, ItemName = item.ItemName, Price = item.Price, Image = filename };
